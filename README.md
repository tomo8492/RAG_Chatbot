...
5. **他PCで「アクセスできません(LAN制限)」と出る場合**: 社内ネットワークが標準のプライベートIP
   (10.x / 172.16〜31.x / 192.168.x)以外(例 `172.36.x.x`)だと弾かれます。`.env` に
   `ALLOWED_CIDRS=172.36.0.0/16` のように**自社のレンジを追加**してください(社外は引き続き遮断されます)
...
- **非標準のプライベートレンジ**(例 `172.36.x.x`。172は16〜31のみがプライベート扱い)を社内で使う場合は、
  `ALLOWED_CIDRS=172.36.0.0/16` のように許可レンジを追加します(指定しないと社内クライアントも弾かれます)。
