...
5. **他PCで「アクセスできません(LAN制限)」と出る場合**: 社内ネットワークが標準のプライベートIP
   (10.x / 172.16〜31.x / 192.168.x)以外だと弾かれます。`172.36.x.x` は既定で許可済みです。
   それ以外のレンジは、`app/main.py` の `EXTRA_PRIVATE_NETWORKS` に追記するか、`.env` の
   `ALLOWED_CIDRS=◯◯/16` で**自社のレンジを追加**してください(社外は引き続き遮断されます)
...
2. **依頼を入力**: 例「README に使い方の章を追記して」「テストを書いて実行して」。AIが `list_files / read_file / write_file / run_command` を使って作業します。
3. **変更の承認**: ファイルの作成・編集やコマンド実行は、**「変更を許可」をオンにし、表示される差分(diff)やコマンドを確認して承認**したときだけ実行されます。オフのあいだは読み取りのみ。
...
- **非標準のプライベートレンジ**(例 `172.36.x.x`。172は16〜31のみがプライベート扱い)を社内で使う場合の許可方法は2通り:
  1. **コードに追記**(恒久・配布物に含める): `app/main.py` の `EXTRA_PRIVATE_NETWORKS` にレンジを足す
     (既定で `172.36.0.0/16` を登録済み。`ipaddress.ip_network("172.50.0.0/16", strict=False)` のように追加)。
  2. **環境変数で指定**(配置ごと): `.env` に `ALLOWED_CIDRS=172.36.0.0/16`(カンマ区切りで複数可)。
  どちらも接続元の実IPで判定し、社外IPは引き続き遮断します。
