...
6. ask_user(質問)は乱用しない。まず「答えが欲しい質問」か「やってほしい作業」かを見極める:
   ...
   - 質問は1つに絞り、「何を決めたいか」を具体的に書く(例:「DBは何を使う?」ではなく
     「ユーザー認証の保存先をどれにしますか?」のように対象と論点を明確に)。
   - 選択肢は2〜4個・互いに排他。各選択肢に label(短い見出し)と
     description(選ぶと何が起きるか・トレードオフを1文)を必ず付ける。
   - 「はい/いいえ」「おまかせ」「その他」など中身のない選択肢にしない(自由記述欄は自動で付く)。
   - 既定として妥当な案があれば、それ1つだけ recommended=true にする。
...
_T_ASK = {"type": "function", "function": {
    ...
    "description": (
        ...
        "質問は1つに絞り、何を決めたいのか具体的に書く。"
        "選択肢は2〜4個・互いに排他で、選ぶと結果が明確に変わるものにし、"
        "各選択肢に label(短い見出し)と description(選ぶと何が起きるか/トレードオフを1文で)を付ける。"
        "妥当な既定があれば1つだけ recommended=true にする。"),
    "parameters": {"type": "object", "properties": {
        "question": {"type": "string", "description": "確認したい具体的な質問(1つ)"},
        "context": {"type": "string",
                    "description": "なぜ確認が必要か・現状の前提(任意・1〜2文)"},
        "options": {"type": "array", "minItems": 2, "maxItems": 4,
            "description": "2〜4個の互いに排他な選択肢",
            "items": {"type": "object", "properties": {
                "label": {"type": "string", "description": "短い見出し(1〜5語)"},
                "description": {"type": "string", "description": "選ぶと何が起きるか・トレードオフ(1文)"},
                "recommended": {"type": "boolean", "description": "推奨ならtrue(任意・最大1つ)"}},
                "required": ["label"]}}},
    ...
    for _ in range(MAX_STEPS):
        ...
        for tc in tool_calls:
            ...
            if name == "ask_user":
                ...
                aid = new_pending()
                ev = {"type": "ask", "action_id": aid, "question": question, "options": options}
                if context:
                    ev["context"] = context
                yield ev
                ans = wait_answer(aid)
                ans = (ans or "").strip() or "(回答なし)"
                # 回答に対応する選択肢の説明があれば付けて、モデルが意図を取り違えないようにする
                picked = next((o for o in options if o["label"] == ans), None)
                ans_full = f"{ans}({picked['description']})" if picked and picked.get("description") else ans
