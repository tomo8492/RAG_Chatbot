...
1. まず glob / grep / read_file で現状を十分に調査する。
   - 変更対象だけでなく、その import 先・呼び出し元・同種の既存実装も read_file で確認し、
     プロジェクトの既存スタイル(命名・エラー処理・テストの書き方)に合わせる。
     1ファイルを直すときは、それを使っている箇所も grep して影響範囲を把握する。
...
3. 承認後(または計画モードでないとき)は、計画に沿って実行する。
   ...
   同じファイルの複数箇所を直すときは edit_file の edits 配列で一括適用する(承認1回・原子的)。
