...
- **2026-06-06 / フェーズ3 完了(サイレント握りつぶしゼロ)**: 「ログも再raiseもしない」except
  75箇所すべてに `log.debug("<関数名>: 例外を無視して継続", exc_info=True)` を付与(挙動不変)。
  ロガー未定義の5ファイル(tools/safety/helpers/config/context)にモジュールロガーを追加。
  - 結果: **サイレント握りつぶし 75 → 0**。全 except が最低 debug で痕跡を残す(DEBUG有効時に追跡可)。
  - 観測性ミドルウェア(相関ID/アクセスログ/未処理例外の集中ログ)と併せ、**fy3 Done基準
    「失敗時に必ず痕跡が残る/サイレントゼロ」を達成**。
  - 検証: ruff 緑 / mypy 21(基準) / 全13テスト緑 / app.main import OK / ASGIスモークOK。
  - 残(任意): 広い `except Exception`(81)の型絞り込み、型付き例外+集中ハンドラ(odysseus流)。
- **2026-06-06 / フェーズ4 完了(フロント分割・export分割・文書)**:
  - **C7**: `app/static/js/app.js`(2,587行)を読み込み順保持で8ファイルへ分割(クラシック script・
    同一グローバルスコープ=**挙動完全不変**。結合が原本と一致することで証明、各ファイル `node --check`)。
    最大ファイルは 664行(code_agent.js)に縮小。インラインハンドラ無しのため将来 ESモジュール化も容易。
  - **C9**: `app/export.py`(1,089行)を package 化。共有の Markdown 解析/インライン整形を
    `export/blocks.py`(242)へ分離、レンダラ+ディスパッチは `export/_render.py`(859)、facade `__init__`。
    `test_export` 31/31 で検証(`__file__` 相対の vendor パス階層も修正)。
  - **C10**: `CONTRIBUTING.md` 追加(`docs/architecture.md` と併せ達成)。
  - 検証: ruff 緑 / mypy 21(基準)/ 全13テスト緑 / 全8エクスポート形式 生成OK。
  - 残(任意): export の per-format さらなる分割、フロントの ESモジュール化(JSテスト整備後)。
