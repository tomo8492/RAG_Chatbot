...
> **状態(2026-06-09 更新):** ③自律検証ループは**簡易版を実装済み**
> (`app/agent/verify.py` + `run_stream` の自動検証ループ。変更後にテスト/ビルドを自動実行→
> 失敗を差し戻して修正→再検証、上限 `MAX_VERIFY_ROUNDS` 回。Codeバーに「検証ループ」トグル+
> 検証コマンド入力)。本書 §4 はより完全な設計(no-progress検出・中止UI・進捗SSE等)で、
> 現状はその簡易版。**①コマンド許可リストは未実装(設計のみ)**。再開時は **§7** から。
